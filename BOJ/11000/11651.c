...
static void	sort_coord(t_coord *coord, int *s_i, int *i)
{
	sorted_coord_x[*s_i] = coord->coord_x[*i];
	sorted_coord_y[*s_i] = coord->coord_y[*i];
	(*s_i)++;
	(*i)++;
}
...
static void	combine(t_coord *coord, int left, int mid, int right)
{
	...
	while (l_i <= mid || r_i <= right)
	{
		if (l_i <= mid && r_i > right)
			sort_coord(coord, &s_i, &l_i);
		else if (l_i > mid && r_i <= right)
			sort_coord(coord, &s_i, &r_i);
		else if (coord->coord_y[l_i] == coord->coord_y[r_i] && \
				coord->coord_x[l_i] <= coord->coord_x[r_i])
			sort_coord(coord, &s_i, &l_i);
		else if (coord->coord_y[l_i] == coord->coord_y[r_i] && \
				coord->coord_x[l_i] > coord->coord_x[r_i])
			sort_coord(coord, &s_i, &r_i);
